...
export const Filter: FunctionComponent<FilterProps> = ({
  ...
  const activeOption = useMemo(() => {
    const found = activeFilterOptions.find(
      (o) => o.filterId === filter.filterId,
    );
    invariant(
      found,
      `Could not find active filter option for filter with id ${filter.filterId}`,
    );
    return found;
  ...
  const filterCustomComponent = useMemo(() => {
    ...
    //     filter.filterId,
    //     customValue,
    //     filterCustomLabelComponent,
    //   ),
    // );

    return (
      <Component updates={[]} setValue={setCustomValue} value={customValue} />
    );
    ...
    >
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <Popover.Content className="w-[150px] p-1" align="start">
          <Command>
            <Command.List className="min-w-90">
