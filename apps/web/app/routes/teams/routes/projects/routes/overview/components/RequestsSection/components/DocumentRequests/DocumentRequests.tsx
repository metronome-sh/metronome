...
export const DocumentRequests: FunctionComponent = () => {
    ...
    <Suspense fallback={<Metric.Skeleton title={title} compact />}>
      <Await resolve={requestsOverview} errorElement={<Metric.Error title={title} compact />}>
        {(resolvedRequestsOverview) => {
          ...
          const value =requestsOverviewEvent?.documentCount ?? resolvedRequestsOverview.documentCount;
          return (
            <Metric
              title={title}
              value={formatNumber(value, '0')}
              rawValue={
                value && value >= 1000 ? `${value?.toLocaleString()} document requests` : undefined
              }
              compact
            />
          );
